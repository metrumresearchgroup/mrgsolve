...
think about the type of outputs we want and determine what inputs we'll need 
to create and the functions that need to be called in order to get those outputs
back.
...
```{r}
mod <- mread_cache("azithro-fixed.mod")
```
...
The first and simplest To get a single profile, use and event object
