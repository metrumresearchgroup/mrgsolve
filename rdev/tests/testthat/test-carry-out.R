...
context("Testing carry.out of tran / pk-dosing items")

ex <- rep(0,7)

test_that("carry.out amt", {
    out <- mod %>% ev(amt=800, rate=2) %>% carry.out(amt) %>% mrgsim
    ex[2] <- 800
    expect_equal(out$amt, ex)
})

test_that("carry.out rate", {
  out <- mod %>% ev(amt=800, rate=2) %>% carry.out(amt,rate,ss) %>% mrgsim
  ex[2] <- 2
...
test_that("carry.out mixed", {
  out <- mod %>% ev(amt=800, rate=2, ii=12, addl=22) %>% carry.out(addl,ii,amt,rate,ss) %>% mrgsim
...
test_that("carry.out mixed, rename", {
...
test_that("carry.out mixed, some rename", {
