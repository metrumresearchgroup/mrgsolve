...
void dataobject:: get_records_pred(recstack& a, int NID, int neq,
  ...
  for(int h=0; h < NID; ++h) {
    ...
    for(j=this->start(h); j <= this->end(h); ++j) {
      ...
      auto obs = std::make_unique<datarecord>(
      ...
      obs->unarm();
      bool is_obs = obs->evid()==0;
      a[h].push_back(std::move(obs));
      if(is_obs) {
        ++obscount;
      } else {
        ++evcount;
  ...
  for(int h=0; h < NID; ++h) {
    ...
    for(j = this->start(h); j <= this->end(h); ++j) {
      ...
      if(Data(j,col[_COL_evid_])==0) {
        ...
        a[h].push_back(std::make_unique<datarecord>(
          Data(j,col[_COL_time_]),
          Data(j,col[_COL_cmt_]),
          j,
          Data(j,Idcol)
      ...
      auto ev = std::make_unique<datarecord>(
        Data(j,col[_COL_cmt_]),
