...
class FlatMatrix {
    ...
    size_t nrow_, ncol_;
public:
    FlatMatrix() : nrow_(0), ncol_(0) {}
    void resize(size_t rows, size_t cols, double val = 0.0) {
        nrow_ = rows;
        ncol_ = cols;
        data_.assign(rows * cols, val);
    }
    double* operator[](size_t row) { return data_.data() + row * ncol_; }
    const double* operator[](size_t row) const { return data_.data() + row * ncol_; }
    size_t nrow() const { return nrow_; }
    size_t ncol() const { return ncol_; }
    size_t size() const { return nrow_; }
};
...
public:
    ...
    size_t idamax1(const double* dx, const size_t n, const size_t offset);

    void dscal1(const double da, double* dx, const size_t n,
                const size_t offset);

    double ddot1(const double* a, const double* b, const size_t n,
                 const size_t offsetA, const size_t offsetB);

    void daxpy1(const double da, const double* dx, double* dy,
                const size_t n, const size_t offsetX,
                const size_t offsetY);

    void dgesl1(const FlatMatrix &a, const size_t n, vector<int> &ipvt,
               vector<double> &b, const size_t job);

    void dgefa1(FlatMatrix &a, const size_t n, vector<int> &ipvt,
               size_t *const info);
    ...
    double fnorm(int n, const FlatMatrix &a, const double* w);
    double vmnorm(const size_t n, const double* v,
                  const double* w);
