...
// ---------------------------------------------------------------------------
// add_operator_spaces: normalise whitespace around binary operators
// ---------------------------------------------------------------------------
//
// Inserts exactly one space on each side of every binary +, -, *, /, and =
// operator.  Two regions are left completely untouched:
//   * pow( ... ) calls — arguments copied verbatim
//   * ** tokens       — convert_pow handles those
// Compound relational operators (==, !=, <=, >=) are treated as a unit and
// also receive one space on each side.  Unary + and - are detected
// heuristically (preceded by an operator, '(', or start-of-line) and receive
// no surrounding spaces.  C++ line comments (// ...) are protected.

static char last_nonspace_char(const std::string& s) {
...
static std::string add_operator_spaces_line(const std::string& line) {
  size_t n = line.size();
  if (n == 0) return line;
  ...
  for (size_t i = 0; i + 1 < n; ++i) {
    if (line[i] == '/' && line[i+1] == '/') {
      for (size_t k = i; k < n; ++k) protect[k] = true;
      break;
    }
  }
  ...
  // Pass 2: protect **
  for (size_t i = 0; i + 1 < n; ++i) {
    if (!protect[i] && line[i] == '*' && line[i+1] == '*') {
      protect[i] = protect[i+1] = true;
    }
  }

  // Pass 3: protect entire pow(...) calls (including the keyword)
  for (size_t i = 0; i < n; ) {
    if (!protect[i] && line[i] == 'p' &&
        (i == 0 || (!std::isalnum((unsigned char)line[i-1]) && line[i-1] != '_')) &&
        n - i >= 4 &&
        line[i+1] == 'o' && line[i+2] == 'w' && line[i+3] == '(') {
      size_t open = i + 3;
      int depth = 1;
      size_t j = open + 1;
      while (j < n && depth > 0) {
      ...
      for (size_t k = i; k < j; ++k) protect[k] = true;
      i = j;
    } else {
      ++i;
    }
  ...
  for (size_t i = start; i < n; ) {
    ...
    // == (two-char operator)
    if (c == '=' && i + 1 < n && !protect[i+1] && line[i+1] == '=') {
      trim_trail(); out += " == "; i += 2; skip_space(i); continue;
    }

    // = that completes !=, <=, >=: retroactively add spaces
    if (c == '=') {
      char prev = last_nonspace_char(out);
      if (prev == '!' || prev == '<' || prev == '>') {
        trim_trail();
        char lead = out.back(); out.pop_back();
        trim_trail();
        out += ' '; out += lead; out += "= ";
        ++i; skip_space(i); continue;
      }
      // Standalone assignment
      trim_trail(); out += " = "; ++i; skip_space(i); continue;
    }
    ...
    if (c == '+' || c == '-' || c == '*' || c == '/') {
      if (c == '+' || c == '-') {
        char prev = last_nonspace_char(out);
        bool unary = (prev == '\0' || prev == '(' ||
                      prev == '+' || prev == '-' ||
                      prev == '*' || prev == '/' || prev == '=');
        if (unary) { out += c; ++i; skip_space(i); continue; }
      }
      trim_trail(); out += ' '; out += c; out += ' '; ++i; skip_space(i); continue;
    }
...
Rcpp::CharacterVector add_operator_spaces_impl(Rcpp::CharacterVector code) {
  Rcpp::CharacterVector out(code.size());
  for (int i = 0; i < code.size(); ++i) {
    out[i] = add_operator_spaces_line(Rcpp::as<std::string>(code[i]));
  }
  return out;
}
