...
test_that("carry_out amt [MRGSOLVE-TEST-0022]", {
    out <- mod %>% ev(amt=800, rate=2) %>% carry_out(amt) %>% mrgsim
    ex[2] <- 800
    expect_equal(out$amt, ex)
})

test_that("carry_out rate [MRGSOLVE-TEST-0023]", {
...
test_that("carry_out mixed [MRGSOLVE-TEST-0024]", {
...
test_that("carry_out mixed, rename [MRGSOLVE-TEST-0025]", {
  out <- mod %>% carry_out(a=addl,i=ii,d=amt,r=rate,s=ss) %>% mrgsim
  expect_equal(names(out), c("ID", "time","d", "s", "i", "a", "r", "CP"))
  expect_equivalent(as.data.frame(out)[2,2:7], list(0,200,1,14,99,11))
})

test_that("carry_out mixed, some rename [MRGSOLVE-TEST-0026]", {
  out <- mod %>% carry_out(a=addl,ii,d=amt,rate,s=ss) %>% mrgsim
